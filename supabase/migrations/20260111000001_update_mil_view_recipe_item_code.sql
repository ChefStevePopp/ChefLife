-- =============================================================================
-- MIGRATION: Update master_ingredients_with_categories view for recipe item_code
-- =============================================================================
-- Date: January 11, 2026
-- Purpose: Pull item_code from recipes table instead of hardcoding NULL
-- 
-- The view unions master_ingredients (purchased) with recipes (prepared).
-- Now that recipes have item_code (P-xxxxxxxx format), we expose it.
-- =============================================================================
...
SELECT
  ...
  COALESCE(mi.ingredient_type, 'purchased')::text as ingredient_type,
  mi.source_recipe_id,
  (
    select
      json_build_object(
...
SELECT
  ...
  r.updated_at,
  r.organization_id,
  r.item_code,  -- NOW PULLING FROM RECIPES TABLE (P-xxxxxxxx format)
  ...
  -- TODO: These should eventually pull from recipe's calculated allergens
...
-- Add comment
COMMENT ON VIEW public.master_ingredients_with_categories IS 
'Unified view of all ingredients: purchased items from master_ingredients table 
UNION with prepared items from recipes table. Prep items now include item_code 
(P-xxxxxxxx format) and source_recipe_id for bidirectional linking.';
