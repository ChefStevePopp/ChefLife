 ...
 * Queued schedules awaiting activation, with search, source filter, and bulk actions.
 *
 * @diagnostics src/features/admin/components/sections/ScheduleManager/components/UpcomingSchedulesView.tsx
...
export const UpcomingSchedulesView: React.FC<UpcomingSchedulesViewProps> = ({
  upcomingSchedules,
  onActivate,
  onExport,
  onDelete,
  ...
  const groupedSchedules = useMemo(() => {
    ...
    const uniqueSchedules = Array.from(
      new Map(upcomingSchedules.map(s => [s.id, s])).values()
    );
    ...
    return Object.values(groups).sort((a, b) => {
      if (a.year !== b.year) return a.year - b.year; // Ascending for upcoming
      return a.month - b.month; // Ascending for upcoming
    });
  }, [upcomingSchedules, searchTerm, sourceFilter]);
  ...
  // Auto-expand the first month (earliest upcoming)
  useMemo(() => {
  ...
  const uniqueSchedules = new Map(upcomingSchedules.map(s => [s.id, s])).size;
    ...
    return (
      <div className="text-center py-12 bg-gray-800/50 rounded-lg">
        ...
        <p className="text-gray-400 max-w-md mx-auto mb-6">
          Upload a new schedule to have it appear here before activating it.
        </p>
                ...
                <div key={key} className="bg-gray-800/30 rounded-lg border border-gray-700/50">
                  ...
                  {isExpanded && (
                    <div className="px-4 pb-4 space-y-3">
                      {group.schedules
                        .sort((a, b) => a.start_date.localeCompare(b.start_date)) // Earliest first for upcoming
                        .map((schedule) => (
                          <div
                            key={schedule.id}
          ...
          <div className="bg-gray-900 rounded-lg w-full max-w-md p-6">
            <div className="text-center">
              ...
              <p className="text-gray-400 mb-6">
                Are you sure you want to delete this upcoming schedule? This action cannot be undone.
              </p>
