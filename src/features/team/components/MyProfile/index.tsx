...
import { 
  ...
  ChevronRight, Edit3, Info, Calendar, TrendingUp
} from "lucide-react";
...
import { MyWeekTab } from "./MyWeekTab";
...
type ProfileTabType = "profile" | "myweek";
...
const ProfileTabContent: React.FC<ProfileTabContentProps> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-[#262d3c] rounded-lg p-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-gray-800/30 rounded-xl p-5 border border-gray-700/30">
          <SectionHeader
            ...
            action={
              <button
                onClick={() => onEditProfile('certifications')}
                className="text-xs text-gray-500 hover:text-gray-300 transition-colors"
              >
                View All
              </button>
        ...
        <div className="bg-gray-800/30 rounded-xl p-5 border border-gray-700/30">
          <SectionHeader
            ...
            action={
              <button
                onClick={() => onEditProfile('notifications')}
                className="text-xs text-gray-500 hover:text-gray-300 transition-colors"
              >
                Manage
              </button>
...
export const MyProfile: React.FC = () => {
  ...
  const [performanceModuleEnabled, setPerformanceModuleEnabled] = useState<boolean | null>(null);

  // Check if Team Performance module is enabled
  useEffect(() => {
    const checkModuleStatus = async () => {
      if (!organizationId) {
        // No org = no module, default to profile tab
        setPerformanceModuleEnabled(false);
        return;
      }
      ...
      try {
        ...
        if (!error && data) {
          ...
          setPerformanceModuleEnabled(isEnabled);
          // If module is enabled, default to My Week tab
          if (isEnabled) {
            setActiveTab('myweek');
          }
        } else {
  ...
  // Tab configuration - L5 color progression
  // Only include My Week tab if module is enabled
  const tabs: TabConfig[] = useMemo(() => {
    const baseTabs: TabConfig[] = [];
    
    if (performanceModuleEnabled) {
      baseTabs.push({ id: "myweek", label: "My Week", icon: Calendar, color: "primary" });
    }
    
    baseTabs.push({ id: "profile", label: "Profile", icon: User, color: performanceModuleEnabled ? "green" : "primary" });
  ...
  // Handle edit profile
  const handleEditProfile = (tab?: string) => {
    setInitialTab(tab);
    setIsModalOpen(true);
  };
  ...
  if (authLoading || isLoading || performanceModuleEnabled === null) {
    return (
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-gray-400 text-sm">
              Your account and weekly performance
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-[#1a1f2b] rounded-lg shadow-lg">
        ...
        <div className="p-4 sm:p-6">
          {activeTab === "myweek" && performanceModuleEnabled && <MyWeekTab />}
          {activeTab === "profile" && (
            <ProfileTabContent
              user={user}
